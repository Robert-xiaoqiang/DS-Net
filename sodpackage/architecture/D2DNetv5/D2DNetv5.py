...
class ComplementaryGatedFusion(nn.Module):
    def __init__(self, multi_scale_inplanes):
        ...
        self.decoders = nn.ModuleList([
            nn.Sequential(
                ...
                BatchNorm2d(2 * p),
                nn.ReLU(inplace = True),
                nn.Conv2d(2 * p, p, 1, stride = 1, padding = 0),
                BatchNorm2d(p),
                nn.ReLU(inplace = True)
            )
    ...
    def forward(self, from_depth_estimation, from_rgb, from_depth_extraction):
        ...
        concated_feature = [ torch.cat([ da_feature[i], gated_fusion_feature[i], dc_feature[i] ], dim = 1) for i in range(self.ns) ]

        y = [ self.decoders[i](concated_feature[i]) for i in range(self.ns) ]
...
class D2DNetv5(nn.Module):
    ...
    def forward(self, rgb, depth):
        ...
        from_depth_extraction = depth_extraction_feature # subnet ????
        
        adaptive_combination = self.cgf(from_depth_estimation, from_rgb, from_depth_extraction)
        
        adaptive_combination = D2DNetv5.merge(adaptive_combination)
