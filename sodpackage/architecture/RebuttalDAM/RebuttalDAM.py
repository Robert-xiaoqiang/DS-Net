...
class DecayedComplementaryGatedFusion(nn.Module):
    def __init__(self, multi_scale_inplanes):
        ...
        self.decayed_dams = nn.ModuleList([nn.Sequential(
                                            nn.Conv2d(2 * p, p, 3, stride = 1, padding = 1),
                                            BatchNorm2d(p),
                                            nn.ReLU())
                                           for p in self.multi_scale_inplanes ])
    
    def forward(self, from_depth_estimation, from_rgb, from_depth_extraction):

        # main
        decayed_da_feature = [ self.decayed_dams[i](torch.cat([ from_rgb[i], from_depth_extraction[i] ], dim = 1)) for i in range(self.ns) ]
        # auxiliary
        ...
        gated_fusion_feature = [ decayed_da_feature[i] + gate_map[i] * dc_feature[i] for i in range(self.ns) ]
...
class RebuttalDAM(nn.Module):
    def __init__(self, config):
        ...
        self.cgf = DecayedComplementaryGatedFusion(last_stage_channels)
    ...
    def forward(self, rgb, depth):
        ...
        adaptive_combination = RebuttalDAM.merge(adaptive_combination)
