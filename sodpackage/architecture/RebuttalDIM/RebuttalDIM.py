...
class DecayedComplementaryGatedFusion(nn.Module):
    def __init__(self, multi_scale_inplanes):
        ...
        self.layers = nn.ModuleList([nn.Sequential(
                                        nn.Conv2d(3 * p, p, 3, stride = 1, padding = 1),
                                        BatchNorm2d(p),
                                        nn.ReLU())
    ...
    def forward(self, from_depth_estimation, from_rgb, from_depth_extraction):
        ...
        y = [ self.layers[i](torch.cat([ from_depth_estimation[i], from_rgb[i], from_depth_extraction[i] ], dim = 1)) for i in range(self.ns) ]
...
class RebuttalDIM(nn.Module):
    ...
    def forward(self, rgb, depth):
        ...
        adaptive_combination = RebuttalDIM.merge(adaptive_combination)
