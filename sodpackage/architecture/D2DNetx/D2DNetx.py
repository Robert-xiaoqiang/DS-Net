...
class FeatureEnhancedModule(nn.Module):
    def __init__(self, inplanes0, inplanes1):
        ...
        self.outplanes = self.inplanes0

        self.encoder0 = nn.Conv2d(self.inplanes0, self.inplanes0, 1, stride = 1, padding = 0)
        self.aspp = ASPP(self.inplanes0, self.inplanes0)
        self.encoder1 = nn.Sequential(
            nn.Conv2d(self.inplanes1, self.inplanes0, 1, stride = 1, padding = 0),
            BatchNorm2d(self.inplanes0),
            nn.ReLU(inplace = True)
        )

        self.decoder = nn.Sequential(
            nn.Conv2d(self.inplanes0, self.outplanes, 1, stride = 1, padding = 0),
            BatchNorm2d(self.outplanes),
            nn.ReLU(inplace = True)
        )
    ...
    def forward(self, f, larger):
        ...
        y = self.decoder(enhanced_f + encoded_larger)
...
class D2DNetx(nn.Module):
    def __init__(self, config):
        ...
        self.fem2 = FeatureEnhancedModule(last_stage_channels[-1], last_stage_channels[-4])

        self.last_layer = nn.Sequential(
            nn.Conv2d(
                in_channels=last_stage_channels[-1],
                out_channels=last_stage_channels[-1],
                kernel_size=1,
                stride=1,
                padding=0),
            BatchNorm2d(last_stage_channels[-1], momentum=BN_MOMENTUM),
            nn.ReLU(inplace=False),
            nn.Conv2d(
                in_channels=last_stage_channels[-1],
                out_channels=1,
